"""
ALL 카테고리 — 폴더 하나의 카테고리를 끝까지 채우는 창.
...
class CategoryDialog(QDialog):
    ...
    def __init__(self, parent, folders: list, on_start=None, on_stop=None,
        ...
        self.btn_run = QPushButton("🗂 카테고리 채우기")
    ...
    def _preview(self):
        ...
        if self.counter and folder:
            try:
                ...
                n = (f"지금 이 폴더에 카테고리 미저장이 "
                     f"**{lcps:,}종 / {rows:,}건** 남아 있습니다." + NL)
            except Exception:
                n = ""
        self.lbl_note.setText(
            n
            + "· 대상은 **상품정보가 「미작업」인 것만**입니다."
              " 대표이미지 승인 여부는 가리지 않습니다." + NL
            + "· 시작하면 먼저 작업상태를 다시 읽습니다(폴더가 크면 몇 분)."
              " 그 다음 LCP 마다 후보를 조회해 카테고리를 정해 저장합니다." + NL
            + "· 카테고리 저장은 **상품정보 상태를 바꾸지 않습니다.**"
              " 「저장완료」로 넘어가지 않습니다.")
    ...
    def _start(self):
        ...
        msg = [
            f"작업폴더 : {folder}",
            "",
            f"대상 : 카테고리 미저장 · 상품정보 미작업  {lcps:,}종 / {rows:,}건",
            "        (대표이미지 상태는 가리지 않습니다)",
            "",
            "하는 일",
            "  1) 작업상태 다시 읽기 (폴더가 크면 몇 분 걸립니다)",
            "  2) LCP 마다 카테고리 후보 조회 · 근거 등급 매기기",
            "  3) 등급별로 한 코드를 정해 저장",
            "  4) 저장분을 되읽어 실제 반영 확인",
            "",
            "카테고리를 저장해도 상품정보 상태는 바뀌지 않습니다.",
            "이미 카테고리가 있는 상품은 건드리지 않습니다.",
            "",
            "시작할까요?",
        ]
        if QMessageBox.question(
                self, "ALL 카테고리", NL.join(msg),
    ...
    def mark_running(self, folder: str, state: dict = None):
        ...
        self.lbl_head.setText(f"{folder} — 카테고리 채우는 중")
        if self._t0 is None:
            self._t0 = time.time()
        if state:
            self.on_stat(state)
